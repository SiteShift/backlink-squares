...
export const metadata = buildMetadata({ title: 'Free SEO Tools: Check Links & Find Opportunities', description: 'Find backlink opportunities, inspect live backlinks, discover internal links and audit crawlability. Free website tools with clear evidence and no signup.', canonicalUrl: 'https://backlinkgrid.com/tools' })
export default function ToolsPage() {
  return <><Header /><main className="pt-14 sm:pt-16 lg:pt-20 bg-surface-50 min-h-screen"><div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
    ...
    <h1 className="text-4xl sm:text-6xl font-bold tracking-tight mt-4 max-w-3xl">Your website.<br />Your next useful finding.</h1>
    <p className="text-lg text-surface-600 mt-6 max-w-2xl">Find relevant backlink opportunities, check real links and uncover technical issues. Start with your website URL. No account required.</p>
    <div className="grid md:grid-cols-2 gap-x-10 mt-12">{liveTools.map((tool, i) => <Link href={`/tools/${tool.slug}`} key={tool.slug} className="group border-t border-surface-300 py-7 flex gap-5"><span className="text-xs text-surface-400 mt-2 tabular-nums">{String(i + 1).padStart(2, '0')}</span><div><h2 className="font-bold text-xl group-hover:text-brand-red transition-colors">{tool.title} ↗</h2><p className="text-sm text-surface-600 mt-3 leading-relaxed">{tool.description}</p><span className="inline-block mt-4 text-sm font-semibold text-brand-red">Open tool →</span></div></Link>)}</div>
    <details className="border-t border-surface-300 py-7"><summary className="font-bold text-xl cursor-pointer">More calculators, templates and CSV tools</summary><div className="grid sm:grid-cols-2 gap-4 mt-6">{freeTools.map(tool => <Link className="underline text-sm text-surface-600 hover:text-brand-red" key={tool.slug} href={`/tools/${tool.slug}`}>{tool.title}</Link>)}</div></details>
