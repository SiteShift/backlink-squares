...
import { getHubContent, getHubClusters, getClusterContent, resolveSiblingPages, resolveCrossHubLinks, resolveRelatedTerms } from '@/lib/content'
...
export default async function BacklinksClusterPage({ params }: PageProps) {
  const { slug } = await params
  const hub = getHubContent(HUB_SLUG)
  const cluster = getClusterContent(HUB_SLUG, slug)
